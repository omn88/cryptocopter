    ...
    async def _convert_to_buy_data(self, position: Position) -> Optional[HPBuyData]:
        ...
        try:
            ...
            logger.debug(
                ...
                position.status, position.strategy_state,
                (position.status == PositionStatus.NEW and 
                 position.strategy_state and 
                 position.strategy_state in ['BUYING', 'SELLING'])
            )
            if (position.status == PositionStatus.NEW and 
                position.strategy_state and 
                position.strategy_state in ['BUYING', 'SELLING']):
                logger.info(
