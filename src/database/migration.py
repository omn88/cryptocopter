...
class DatabaseMigration:
    """
    Handles migration from old MySQL database to new SQLite database.    This migration:
    ...
    async def migrate_all_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.new_db.backup_database(
                f"backup_before_migration_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
            )
            ...
            # Initialize summary if not already done
            ...
            for old_strategy in old_strategies:
                strategy = Strategy(
                    ...
                    updated_at=self._parse_datetime(
                        old_strategy.get("version_timestamp")
                    ),
                )
    ...
    async def _convert_hp_record_to_position(
        self, hp_record: Dict
    ) -> Optional[Position]:
    ...
    async def _convert_buy_level_to_position(
        self, buy_level: Dict
    ) -> Optional[Position]:
    ...
    async def _convert_sell_level_to_position(
        self, sell_level: Dict
    ) -> Optional[Position]:
    ...
    async def validate_migration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            new_stats = (
                await self.new_db.get_database_stats()
            ...
            if new_stats.get("active_positions", 0) == 0:
                validation_report["issues"].append(
                    "No active positions found in new database"
                )
...
async def run_migration(
    old_db_config: Dict, new_db_path: str = "trading.db"
) -> Dict[str, Any]:
    """
