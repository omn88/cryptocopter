...
class TradingDatabase:
    ...
    async def get_orders_by_hp_id(
        self, hp_id: str, side: Optional[str] = None
    ) -> List[Order]:
        """
        Get all orders for a position by HP ID.
        ...
        Returns:
            List of orders for the HP position
        """
        try:
            async with self.get_connection() as conn:
                if side:
                ...
                for row in rows:
                    order = Order(
                        ...
                        side=row["side"],
                        order_type=row["order_type"],
                        status=self._convert_order_status_string(row["status"]),
                        price=row["price"],
                        quantity=row["quantity"],
                        quantity_stable=row["quantity_stable"],
                        realized_quantity=row["realized_quantity"],
            ...
            logger.error("Failed to get orders by hp_id %s: %s", hp_id, e)
            return []
