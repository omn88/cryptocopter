...
async def test_complete_hp_lifecycle_portfolio_communication(portfolio_ui: PortfolioUI):
    ...
    # Debug: Check initial state from mock_inventory (should be 4 items)
    logger.info("Initial inventory: %s", len(portfolio_ui.inventory))

    assert len(portfolio_ui.inventory) == 4  # 4 original
    ...
    assert len(portfolio_ui.inventory) == 5  # 4 original + 1 new HP buy
    ...
    # Should have 4 inventory items (original 4 + 1 HP item for hp_lifecycle_001 with aggregated quantities)
    ...
    # Should have 6 inventory items (original 4 + 1 for hp_lifecycle_001 + 1 for hp_lifecycle_001_additional)
