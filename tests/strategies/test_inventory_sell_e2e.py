...
#         """Get inventory item for a specific coin."""
#         inventory = self.strategy_executor.inventory
...
#     # Verify we have inventory items available
#     assert (
#         len(strategy_executor.inventory) > 0
#     ), "Should have inventory items for testing"

#     # Verify inventory contains expected items (from mock_inventory fixture)
#     inventory_coins = [item.coin for item in strategy_executor.inventory]
#     expected_coins = ["BTC", "ETH", "AXL"]  # Based on mock_inventory fixture

#     for coin in expected_coins:
#         assert coin in inventory_coins, f"Inventory should contain {coin}"

#     # Verify inventory items have the expected structure
#     btc_item = next(item for item in strategy_executor.inventory if item.coin == "BTC")
#     assert btc_item.available_quantity > 0, "BTC item should have positive quantity"
#     assert btc_item.avg_buy_price > 0, "BTC item should have positive buy price"

#     logger.info(
#         f"Verified inventory setup with {len(strategy_executor.inventory)} items"
#     )
...
#     assert hasattr(
#         portfolio, "hp_manager"
#     ), "Portfolio should have hp_manager reference"
#     assert hasattr(
#         hp_manager, "portfolio"
#     ), "HP manager should have portfolio reference"
#     assert (
#         portfolio.hp_manager is hp_manager
#     ), "Portfolio should reference the correct HP manager"
#     assert (
#         hp_manager.portfolio is portfolio
#     ), "HP manager should reference the correct portfolio"

#     # Verify backend connections
#     assert (
#         hp_manager.config_queue is strategy_executor.config_queue
#     ), "HP manager should use strategy executor config queue"
#     assert (
#         strategy_executor.ui_queue is hp_manager.ui_queue
...
#         expected_buy_price=btc_item.avg_buy_price,
#     )
...
#         expected_buy_price=eth_item.avg_buy_price,
#     )

#     logger.info(f"Sell button test passed for ETH item: {eth_item}")


# async def test_inventory_sell_button_click_axl_convert(portfolio_hp_backend_setup):
#     """Test clicking sell button on AXL inventory item for convert sell."""
#     portfolio, hp_manager, strategy_executor = portfolio_hp_backend_setup
#     sim = InventorySellSimulator(portfolio, hp_manager, strategy_executor)

#     # Get AXL inventory item
#     axl_item = sim.get_inventory_item("AXL")

#     # Simulate sell button click
#     result = await sim.simulate_sell_button_click("AXL")

#     # Verify sell modal opened with correct data
#     await sim.verify_sell_modal_opened(
#         coin="AXL",
#         expected_quantity=axl_item.available_quantity,
#         expected_buy_price=axl_item.avg_buy_price,
#     )

#     logger.info(f"Sell button test passed for AXL item: {axl_item}")
...
# async def test_inventory_sell_configure_convert_only_axl_to_usdc(
#     portfolio_hp_backend_setup,
# ):
#     """Test configuring convert-only sell from AXL to USDC."""
#     portfolio, hp_manager, strategy_executor = portfolio_hp_backend_setup
#     sim = InventorySellSimulator(portfolio, hp_manager, strategy_executor)

#     # Start sell flow
#     await sim.simulate_sell_button_click("AXL")

#     # Configure convert sell
#     await sim.configure_convert_sell(end_currency="USDC")
