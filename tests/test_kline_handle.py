...
async def test_rsi_basic_handle_kline_long_ext_long_null_short_ext_short_null(
    ...
    assert NUMBER_OF_DCA_ORDERS == len(special_rsi.position.orders)
    assert 1000 == special_rsi.strategy.balance
    ...
    assert NUMBER_OF_DCA_ORDERS == len(special_rsi.strategy.position.orders)
    assert 1000 == special_rsi.strategy.balance
    assert special_rsi.strategy.position.state == State.LONG
...
async def test_rsi_basic_handle_kline_long_ext_long_null_short_ext_short_null_long_special(
    ...
    assert len(special_rsi.strategy.position.orders) == NUMBER_OF_DCA_ORDERS
    assert 1000 == special_rsi.strategy.balance
    assert special_rsi.strategy.position.state == State.LONG_EXT
    ...
    assert 1 == len(special_rsi.strategy.position.orders)
    assert 1000 == special_rsi.strategy.balance
